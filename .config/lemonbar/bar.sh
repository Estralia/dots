...
GPUTemp() {
    ...
    icon=""
    printf "%s" "%{B$color03}%{F-}%{O7}$icon%{O7}%{B$bg}%{F$fg}%{O7}GPU: $cmd C%{O7}%{B-}"
}
CPUTemp() {
	cmd=$(sysctl -n dev.cpu.1.temperature | sed 's/C//' | sort -nr | head -n1)
	icon=""
	printf "%s" "%{B$color04}%{F-}%{O7}$icon%{O7}%{B$bg}%{F$fg}%{O7}CPU: $cmd C%{O7}%{B-}"
}
while [ "$(pgrep -x lemonbar)" ]; do
	# Text
	left="%{l}%{O4}$(User)"
	center="%{c}"
	right="%{r}$(CPUTemp)%{O5}$(GPUTemp)%{O5}$(Date)%{O4}$(Time)"
	printf "%s\n" "$left$center$right"
